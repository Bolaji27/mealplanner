...
export default function Signin() {
    const [error, actionForm] = useActionState(signInAction, "undefined")
  return (
    <div className="xl:flex lg:flex md:flex w-full flex-col mt-3 ">
      <div className="flex  flex-col xl:flex-row lg:flex-row md:flex-row  xl:w-full lg:w-full md:w-full sm:w-full xl:gap-[20px] lg:gap-[20px] md:gap-[20px] mb-6 sm:flex-col w-full ">
        <Image src={accImage} alt="account image" className="md:w-[346px] xl:w-full xl:h-[559px] lg:w-[486px] lg:h-[570px] md:h-[537px] sm:h-[195px] w-full"/>
        <div className="xl:w-full xl:ml-[98px] text-black text-center lg:w-full md:w-full  ">
          ...
          <form action={actionForm}>
            <div className="flex flex-row  items-center border pl-2 gap-4 font-lowan rounded-md mb-5 border-black h-[52px] pr-2">
              ...
              <label htmlFor="email" />
              <input id="email" name="email" placeholder="Email" type="email" className=" xl:text-[20px] lg:text-[20px] w-full md:text-[16px] sm:text-[16px] text-[16px]"/>
            </div>
            <div className="flex flex-row  items-center border pl-2 justify-between px-2 font-lowan text-[20px] rounded-md border-black h-[52px] w-full">
              <div className="flex flex-row  items-center  gap-4  w-full">
                ...
                <input id="password" name="password" type="password" placeholder="Password"  className="w-full xl:text-[20px] lg:text-[20px] md:text-[16px] sm:text-[16px] text-[16px]"/>
              </div>
              <PiEyeSlashLight className="w-[22px]"/>
            </div>
            <p className="flex justify-end font-lowan text-[16px] mb-6 md:text-[16px]">forgot password ?</p>
            <button className=" border w-full font-hammersmith xl:text-[32px] xl:h-[52px] lg:text-[32px] mb-10 rounded-md border-black md:text-[24px] sm:text-[24px] text-2xl" type="submit">Login</button>
            <p>{error}</p>
          </form>
